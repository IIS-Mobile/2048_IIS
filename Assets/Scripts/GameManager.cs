...
public class GameManager : MonoBehaviour {
	...
	bool MakeOneMoveDownIndex(Tile[] LineOfTiles)
	{
		for (int i =0; i< LineOfTiles.Length-1; i++) 
		{
			...
			if (LineOfTiles[i].Number!= 0 && LineOfTiles[i].Number == LineOfTiles[i+1].Number &&
			    LineOfTiles[i].hasMergedThisTurn == false && LineOfTiles[i+1].hasMergedThisTurn == false)
			{
				LineOfTiles[i].Number *= 2;
				LineOfTiles[i+1].Number = 0;
				LineOfTiles[i].hasMergedThisTurn = true;
    ...
    public void StoreButtonHandler()
    {
        StorePanel.SetActive(true);
    }

    public void StoreExitButtonHandler()
    {
